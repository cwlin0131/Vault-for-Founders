...
用 `YYYY-MM-topic/` 格式，這樣 Agent 跟你都能一眼排序找到。通用範例：

```
projects/
├── YYYY-MM-launch-prep/        ← 例：你正在協調的產品上線
├── YYYY-MM-conference-trip/    ← 例：多日的 networking trip
├── YYYY-MM-fundraising/        ← 例：投資人 outreach + deck 迭代
└── YYYY-MM-partner-collab/     ← 例：跟另一團隊的合作項目
```
