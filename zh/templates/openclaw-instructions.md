...
> 最後更新：v1 —— 從 cowork-instructions.md 拆出，反映 OpenClaw 的不同寫入範圍
...
如果你有 `[你的 Vault 名稱]/skills/` 並放了東西，OpenClaw 會優先讀取你的 skills（蓋過內建的）。這讓你可以覆蓋或擴充特定 skill（例如你自己客製的 skill 優先）。Vault 沒有的 skill 會自動 fallback 到內建版。
